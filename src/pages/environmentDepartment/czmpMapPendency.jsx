...
const CZMPMapPendency = () => {
	...
	const calculatedTotals = useMemo(() => {
		return data.reduce((totals, row) => {
			...
			columns.forEach(col => {
				totals[col.key] = (totals[col.key] || 0) + (row[col.key] || 0);
			});
			return totals;
		}, {
			pendingProposal: 0,
	...
	const handleAddRow = () => {
		...
		const newRow = {
			id: newId,
			srNo: newSrNo,
			agency: newRowData.agency || "New Agency",
			pendingProposal: newRowData.pendingProposal || 0,
		...
		<div className="max-w-7xl mx-auto p-6">
			<div className="mb-6">
				...
				<p className="text-slate-600">
					Track pending proposals across different agencies and their timelines.
				</p>
			</div>
			...
			<div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
					...
					<table className="w-full text-left align-middle min-w-[1000px] border border-slate-300">
						<thead className="bg-gradient-to-r from-[#dcf2ff] to-sky-100/70 text-slate-700 text-xs uppercase tracking-wide">
							<tr>
								...
								<th className="px-4 py-3 whitespace-nowrap w-[200px] border border-slate-300">Agency NAME</th>
				...
				<div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50">
					<div className="flex items-center gap-2 text-xs text-slate-500">
						<span>
							Showing {filtered.length} of {data.length} agencies
						</span>
						{query && (
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
					<div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
						...
						<p className="text-slate-700 mb-6">
							Are you sure you want to delete this agency from the CZMP Map Pendency table?
						</p>
...
export default CZMPMapPendency;