...
const initialData = [
	{
		id: 1,
		srNo: 1,
    caseTitle: "OA/77 of 2023 (WZ) Banda Nagaraj Kumar and Anr V/s Maharashtra Maritime Board and Ors",
    dateOfFinalOrder: "9/16/2025",
		complianceExpectedFrom: "Y",
		complianceStatus: "Pending",
		timelineForCompliance: "2 months",
		expectedDateOfCompliance: "11/15/2025",
		delayDays: "0",
		concernedDepartment: "ENGINEERING",
		contemptPetitionFiled: "-",
		statusOfContempt: "-"
	},
	{
		id: 2,
		srNo: 2,
		caseTitle: "WP/3543 of 2021 Sushil Ratnakar Nimbalkar V/s The State Off Maharashtra ",
		dateOfFinalOrder: "8/27/2021",
		complianceExpectedFrom: "N",
		complianceStatus: "N.A.",
		timelineForCompliance: "N.A.",
		expectedDateOfCompliance: "N.A.",
		delayDays: "0",
		concernedDepartment: "ENGINEERING",
		contemptPetitionFiled: "CP/264/2022",
		statusOfContempt: "Pending at Pre"
	},
	{
		id: 3,
		srNo: 3,
		caseTitle: "PIL/49 of 2013 Parshuram uparkar v/s MMB &Ors ",
		dateOfFinalOrder: "10/22/2013",
		complianceExpectedFrom: "N",
		complianceStatus: "N.A.",
		timelineForCompliance: "N.A.",
		expectedDateOfCompliance: "N.A.",
		delayDays: "0",
		concernedDepartment: "PLANNING",
		contemptPetitionFiled: "CP/ 172/2015",
		statusOfContempt: "Pending at Pre"
	}
];

const columns = [
	{ key: "srNo", label: "Sr No", width: "80px", align: "center" },
	{ key: "caseTitle", label: "Case Title", width: "200px" },
	{ key: "dateOfFinalOrder", label: "Date of Final Order", width: "120px", align: "center" },
	{ key: "complianceExpectedFrom", label: "Compliance expected from MMB (Y/N)", width: "120px", align: "center" },
	{ key: "complianceStatus", label: "Compliance Status", width: "120px", align: "center" },
	{ key: "timelineForCompliance", label: "Timeline for Compliance", width: "120px", align: "center" },
	{ key: "expectedDateOfCompliance", label: "Expected Date of Compliance", width: "120px", align: "center" },
	{ key: "delayDays", label: "Delay Days", width: "80px", align: "center" },
	{ key: "concernedDepartment", label: "Concerned Department", width: "120px" },
	{ key: "contemptPetitionFiled", label: "Contempt Petition filed", width: "120px" },
	{ key: "statusOfContempt", label: "Status of Contempt petition", width: "120px" }
];

const FinalCourtOrders = () => {
	...
	// Filter logic: Search by Case Title
	...
	// --- Modal Component ---
	const DeleteConfirmationModal = ({ show, onClose, onConfirm, caseTitle }) => {
		if (!show) return null;
		return (
			<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
				<div className="bg-white rounded-lg shadow-xl p-6 max-w-sm w-full">
					<h2 className="text-lg font-bold mb-2">Delete Row</h2>
					<p className="mb-4">Are you sure you want to delete <span className="font-semibold">{caseTitle}</span>?</p>
		...
		<div className="max-w-7xl mx-auto p-6">
			<div className="mb-6">
				<h1 className="text-2xl font-bold text-slate-900 mb-2 ">
				 3. COMPLIANCE STATUS OF DIRECTIONS UNDER FINAL COURT ORDERS
				</h1>
			</div>
					...
					<table className="w-full text-left align-middle min-w-[1000px] border border-slate-300">
						<thead className="bg-gradient-to-r from-[#dcf2ff] to-sky-100/70 text-slate-700 text-xs uppercase tracking-wide">
							<tr>
								{columns.map((col) => (
									<th
										key={col.key}
										className={`px-4 py-3 whitespace-nowrap border border-slate-300${col.align ? ` text-${col.align}` : ""}`}
										style={{ width: col.width }}
									>
										{col.label}
									</th>
								))}
								<th className="px-4 py-3 whitespace-nowrap border border-slate-300 text-center">Actions</th>
							</tr>
						</thead>
						<tbody className="bg-white">
							{filtered.map((row, idx) => (
								<tr key={row.id} className={`transition-colors group ${idx % 2 === 0 ? 'bg-white' : 'bg-sky-50/40'} hover:bg-sky-50/70`}>
									{columns.map((col) => (
										<td
											key={col.key}
											className={`px-4 py-3 border border-slate-300${col.align ? ` text-${col.align}` : ""}`}
										>
											{editingId === row.id ? (
												<input
													type="text"
							...
							<tr className="font-bold bg-slate-100">
								...
								<td className="px-4 py-3 text-slate-900 font-bold border border-slate-300" colSpan={columns.length - 1}>{data.length} cases</td>
									...
									<td colSpan={columns.length + 1} className="px-4 py-8 text-center bg-white">
										<div className="mx-auto max-w-md">
											<p className="text-slate-600">
												No cases match your search. Try a different query.
											</p>
										</div>
									</td>
								</tr>
			...
			<DeleteConfirmationModal
				...
				caseTitle={data.find(d => d.id === deleteId)?.caseTitle || 'this case'}
			/>
		</div>
	);
};

export default FinalCourtOrders;
