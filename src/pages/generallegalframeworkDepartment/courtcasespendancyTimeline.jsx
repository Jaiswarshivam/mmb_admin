...
const initialCourtTimelineData = [
	{ srNo: 1, court: "Supreme Court", total: 6, upto6: 0, sixTo1: 2, oneTo2: 1, twoTo3: 2, threeAbove: 1 },
	{ srNo: 2, court: "High Court", total: 40, upto6: 4, sixTo1: 1, oneTo2: 7, twoTo3: 7, threeAbove: 21 },
	{ srNo: 3, court: "National Green Tribunal", total: 3, upto6: 0, sixTo1: 0, oneTo2: 1, twoTo3: 1, threeAbove: 1 },
	{ srNo: 4, court: "Industrial Tribunal", total: 1, upto6: 0, sixTo1: 0, oneTo2: 0, twoTo3: 0, threeAbove: 1 },
	{ srNo: 5, court: "District Level Court", total: 35, upto6: 3, sixTo1: 0, oneTo2: 3, twoTo3: 0, threeAbove: 29 },
];
...
const CourtCasesPendencyTimeline = () => {
	const [data, setData] = useState(initialCourtTimelineData);
		...
		<div className="max-w-7xl mx-auto p-6">
			<div className="mb-6">
				<h1 className="text-2xl font-bold text-slate-900 mb-2">
					1. LIST OF ONGOING COURT CASES (Pendency Timeline)
				</h1>
			</div>
				...
				<table className="w-full text-left align-middle min-w-[1000px] border border-slate-300">
					<thead className="bg-gradient-to-r from-[#dcf2ff] to-sky-100/70 text-slate-700 text-xs uppercase tracking-wide">
						<tr>
							...
							<th className="px-4 py-3 whitespace-nowrap w-[120px] text-center border border-slate-300" rowSpan={2}>Total No. of Court Cases</th>
							<th className="px-4 py-3 text-center border border-slate-300" colSpan={5}>Pendency Timeline</th>
							<th className="px-4 py-3 whitespace-nowrap w-[80px] text-center border border-slate-300" rowSpan={2}>Actions</th>
						</tr>
						<tr className="bg-slate-100">
							<th className="px-4 py-3 whitespace-nowrap w-[80px] text-center border border-slate-300">Upto 6 months</th>
							<th className="px-4 py-3 whitespace-nowrap w-[80px] text-center border border-slate-300">6 m. To &lt; 1 Yr</th>
							<th className="px-4 py-3 whitespace-nowrap w-[80px] text-center border border-slate-300">1 To &lt; 2 Yr.</th>
							<th className="px-4 py-3 whitespace-nowrap w-[80px] text-center border border-slate-300">2 To &lt; 3 Yr.</th>
							<th className="px-4 py-3 whitespace-nowrap w-[80px] text-center border border-slate-300">3 Yrs. Above</th>
						</tr>
					</thead>
					<tbody className="bg-white">
						{filteredData.map((row, idx) => (
							<tr key={row.srNo} className={`transition-colors group ${idx % 2 === 0 ? 'bg-white' : 'bg-sky-50/40'} hover:bg-sky-50/70`}>
								...
								<td className="px-4 py-3 text-center border border-slate-300 font-semibold text-slate-900">
									{editingId === row.srNo ? (
										<input type="number" value={editRow.upto6} onChange={e => handleInputChange('upto6', e.target.value)} className="w-full px-2 py-1 text-sm border border-black rounded bg-white text-black text-center" />
									) : row.upto6}
								</td>
								<td className="px-4 py-3 text-center border border-slate-300 font-semibold text-slate-900">
									{editingId === row.srNo ? (
										<input type="number" value={editRow.sixTo1} onChange={e => handleInputChange('sixTo1', e.target.value)} className="w-full px-2 py-1 text-sm border border-black rounded bg-white text-black text-center" />
									) : row.sixTo1}
								</td>
								<td className="px-4 py-3 text-center border border-slate-300 font-semibold text-slate-900">
									{editingId === row.srNo ? (
										<input type="number" value={editRow.oneTo2} onChange={e => handleInputChange('oneTo2', e.target.value)} className="w-full px-2 py-1 text-sm border border-black rounded bg-white text-black text-center" />
									) : row.oneTo2}
								</td>
								<td className="px-4 py-3 text-center border border-slate-300 font-semibold text-slate-900">
									{editingId === row.srNo ? (
										<input type="number" value={editRow.twoTo3} onChange={e => handleInputChange('twoTo3', e.target.value)} className="w-full px-2 py-1 text-sm border border-black rounded bg-white text-black text-center" />
									) : row.twoTo3}
								</td>
								<td className="px-4 py-3 text-center border border-slate-300 font-semibold text-slate-900">
									{editingId === row.srNo ? (
										<input type="number" value={editRow.threeAbove} onChange={e => handleInputChange('threeAbove', e.target.value)} className="w-full px-2 py-1 text-sm border border-black rounded bg-white text-black text-center" />
									) : row.threeAbove}
								</td>
						...
						<tr className="font-bold bg-slate-100">
							...
							<td className="px-4 py-3 text-slate-900 font-bold text-center border border-slate-300">{filteredData.reduce((sum, r) => sum + (parseInt(r.upto6) || 0), 0)}</td>
							<td className="px-4 py-3 text-slate-900 font-bold text-center border border-slate-300">{filteredData.reduce((sum, r) => sum + (parseInt(r.sixTo1) || 0), 0)}</td>
							<td className="px-4 py-3 text-slate-900 font-bold text-center border border-slate-300">{filteredData.reduce((sum, r) => sum + (parseInt(r.oneTo2) || 0), 0)}</td>
							<td className="px-4 py-3 text-slate-900 font-bold text-center border border-slate-300">{filteredData.reduce((sum, r) => sum + (parseInt(r.twoTo3) || 0), 0)}</td>
							<td className="px-4 py-3 text-slate-900 font-bold text-center border border-slate-300">{filteredData.reduce((sum, r) => sum + (parseInt(r.threeAbove) || 0), 0)}</td>
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 transition-opacity">
					<div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl scale-100">
						...
						<p className="text-slate-700 mb-6 pl-12">
							Are you sure you want to delete this row from the Court Cases Pendency Timeline table?
						</p>
...
export default CourtCasesPendencyTimeline;
