...
const initialData = [
	{ id: 1, srNo: 1, portGroup: "Bandra Group of Ports", revenue: { "2022-2023": 1608601, "2023-2024": 1139291, "2024-2025": 891387 } },
	{ id: 2, srNo: 2, portGroup: "Mora Group of Ports", revenue: { "2022-2023": 18880, "2023-2024": 146320, "2024-2025": 42480 } },
	{ id: 3, srNo: 3, portGroup: "Rajpuri Group of Ports", revenue: { "2022-2023": 28000, "2023-2024": 11020, "2024-2025": 1600 } },
	{ id: 4, srNo: 4, portGroup: "Ratnagiri Group of Ports", revenue: { "2022-2023": "Nil", "2023-2024": "Nil", "2024-2025": "Nil" } },
	{ id: 5, srNo: 5, portGroup: "Vengurla Group of Ports", revenue: { "2022-2023": "Nil", "2023-2024": "Nil", "2024-2025": "Nil" } }
];
				...
				<table className="w-full text-left align-middle min-w-[800px] border border-slate-300">
					<thead className="bg-gradient-to-r from-[#dcf2ff] to-sky-100/70 text-slate-700 text-xs uppercase tracking-wide">
						<tr>
							...
							<th className="px-4 py-3 whitespace-nowrap w-[220px] border border-slate-300">PORT GROUP</th>
					...
					<tbody className="bg-white">
						{filtered.map((row, idx) => (
							<tr key={row.id} className={`transition-colors group ${idx % 2 === 0 ? 'bg-white' : 'bg-sky-50/40'} hover:bg-sky-50/70`}>
								...
								<td className="px-4 py-3 border border-slate-300 font-medium text-slate-700">
									{editingId === row.id ? (
										<input type="text" value={editData.portGroup} onChange={e => setEditData(prev => ({ ...prev, portGroup: e.target.value }))} className="w-full px-2 py-1 text-sm border border-black rounded bg-white text-black focus:ring-2 focus:ring-sky-100 focus:border-sky-300" />
									) : row.portGroup}
								</td>
								{revenueYears.map(year => (
						...
						{filtered.length === 0 && (
							<tr>
								<td colSpan={3 + revenueYears.length} className="px-4 py-8 text-center bg-white">
